 ...
 */
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT( FNTTNiagaraTextUniformParameters, NIAGARATEXTTOOLKITVERTEXFACTORY_API)
	...
	SHADER_PARAMETER(int, CharacterIndexDataOffset)
	...
	SHADER_PARAMETER(float, DefaultCharacterIndex)
...
typedef TUniformBufferRef<FNTTNiagaraTextUniformParameters> FNTTNiagaraTextUniformBufferRef;

BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FNTTNiagaraTextVFLooseParameters, NIAGARATEXTTOOLKITVERTEXFACTORY_API)
...
typedef TUniformBufferRef<FNTTNiagaraTextVFLooseParameters> FNTTNiagaraTextVFLooseParametersRef;

/**
* Base class for particle vertex factories.
*/
class FNTTNiagaraTextVertexFactoryBase : public FVertexFactory
{
public:
	/** Default constructor. */
	explicit FNTTNiagaraTextVertexFactoryBase(ENiagaraVertexFactoryType Type, ERHIFeatureLevel::Type InFeatureLevel)
 ...
 */
class FNTTNiagaraTextVertexFactory : public FNTTNiagaraTextVertexFactoryBase
{
public:
	DECLARE_VERTEX_FACTORY_TYPE_API(FNTTNiagaraTextVertexFactory, NIAGARATEXTTOOLKITVERTEXFACTORY_API);

	/** Default constructor. */
	FNTTNiagaraTextVertexFactory(ENiagaraVertexFactoryType InType, ERHIFeatureLevel::Type InFeatureLevel)
		: FNTTNiagaraTextVertexFactoryBase(InType, InFeatureLevel)
	...
	FNTTNiagaraTextVertexFactory()
		: FNTTNiagaraTextVertexFactoryBase(NVFT_MAX, ERHIFeatureLevel::Num)
		, LooseParameterUniformBuffer(nullptr)
		, AlignmentMode(0)
		, FacingMode(0)
		, SortedIndicesSRV(nullptr)
	...
	NIAGARATEXTTOOLKITVERTEXFACTORY_API virtual void InitRHI(FRHICommandListBase& RHICmdList) override;
	 ...
	 */
	static NIAGARATEXTTOOLKITVERTEXFACTORY_API bool ShouldCompilePermutation(const FVertexFactoryShaderPermutationParameters& Parameters);

	/**
	 * Can be overridden by FVertexFactory subclasses to modify their compile environment just before compilation occurs.
	 */
	static NIAGARATEXTTOOLKITVERTEXFACTORY_API void ModifyCompilationEnvironment(const FVertexFactoryShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment);

	/**
	* Get vertex elements used when during PSO precaching materials using this vertex factory type
	*/
	static NIAGARATEXTTOOLKITVERTEXFACTORY_API void GetPSOPrecacheVertexFetchElements(EVertexInputStreamType VertexInputStreamType, FVertexDeclarationElementList& Elements);

	NIAGARATEXTTOOLKITVERTEXFACTORY_API void SetTexCoordBuffer(const FVertexBuffer* InTexCoordBuffer);
	
	/**
	 * Set the uniform buffer for this vertex factory.
	 */
	FORCEINLINE void SetSpriteUniformBuffer( const FNTTNiagaraTextUniformBufferRef& InSpriteUniformBuffer )
	{
...
protected:
	...
	NIAGARATEXTTOOLKITVERTEXFACTORY_API void InitStreams();
